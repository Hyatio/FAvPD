...
from transformers import (
    BertConfig, BertTokenizer, BertForMultipleChoice, 
    RobertaConfig, RobertaTokenizer, RobertaForMultipleChoice,
    get_linear_schedule_with_warmup
)
from process.datasets.SWAGProcessor import SWAGProcessor
from process.metrics.swag_metric import compute_metric
import wandb

MODEL_CLASS = {
    'bert': (BertConfig, BertTokenizer, BertForMultipleChoice),
    'roberta': (RobertaConfig, RobertaTokenizer, RobertaForMultipleChoice)
}
...
def evaluate(model, eval_dataloader, args):
    ...
    for sample in tqdm(eval_dataloader, desc="Eval Iteration"):
        ...
        token_type_ids = sample[2].to(args.device)
        labels = sample[3].to(args.device)
        input_ids = input_ids.reshape(args.train_batch_size*args.num_labels, -1)
        with torch.no_grad():
...
def main():
    ...
    data_processor = SWAGProcessor(args, tokenizer)
        ...
        for step, sample in enumerate(tqdm(train_dataloader, desc="Train Iteration")):
            ...
            if (step+1) % args.gradient_accumulation_steps == 0:
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.wandb_project:
                        wandb.log({'eval loss': eval_loss, 'accuracy': result['accuracy']})
