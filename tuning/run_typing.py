...
from process.models.model import BertForEntityTyping, RobertaForEntityTyping
from process.metrics.typing_metric import compute_metric
import wandb
from peft import LoraConfig, get_peft_model, TaskType

MODEL_CLASS = {
    'bert': (BertConfig, BertTokenizer, BertForSequenceClassification, BertForEntityTyping),
    'roberta': (RobertaConfig, RobertaTokenizer, RobertaForSequenceClassification, RobertaForEntityTyping)
}
...
def evaluate(model, eval_dataloader, args):
    ...
    for sample in tqdm(eval_dataloader, desc="Eval Iteration"):
        ...
        ent_mask = sample[3].to(args.device)
        with torch.no_grad():
            if args.add_entity_embedding:
                outputs = model(input_ids, attention_mask, labels, ent_mask)
            else:
    ...
    # best_result = None
    # best_threshold = .0
    # for threshold in range(-20,20):
    #     args.threshold = threshold/10
    #     result = compute_metric(args, golds, preds)
    #     temp = result['micro-f1'] if args.task_name == 'OpenEntity' \
    #         else result['loose-micro']
    #     if temp > best:
    #         best = temp
    #         best_result = result
    ...
    args.threshold = 0.0
    best_result = compute_metric(args, golds, preds)
    return eval_loss, best_result
        ...
        for step, sample in enumerate(tqdm(train_dataloader, desc="Train Iteration")):
            ...
            if (step+1) % args.gradient_accumulation_steps == 0:
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    metric = result['micro-f1'] if args.task_name == 'OpenEntity' else result['loose-micro']
                    ...
                    if args.wandb_project:
                        if args.task_name == 'OpenEntity':
                            ...
                            wandb.log({'eval loss': eval_loss,
                                        'accuracy': result['accuracy'],
                                        'loose-macro': result['loose-macro'],
                                        'loose-micro': result['loose-micro']})
