...
from process.models.model import BertForRelationClassification, RobertaForRelationClassification
from process.metrics.re_metric import compute_metric
import wandb

MODEL_CLASS = {
    'bert': (BertConfig, BertTokenizer, BertForSequenceClassification, BertForRelationClassification),
    'roberta': (RobertaConfig, RobertaTokenizer, RobertaForSequenceClassification, RobertaForRelationClassification)
}
...
def evaluate(model, eval_dataloader, args):
    ...
    for sample in tqdm(eval_dataloader, desc="Eval Iteration"):
        ...
        e1_mask = sample[3].to(args.device)
        e2_mask = sample[4].to(args.device)
        with torch.no_grad():
            if args.add_entity_embedding:
                outputs = model(input_ids, attention_mask, labels, e1_mask, e2_mask)
            else:
...
def main():
    ...
    data_processor = REProcessor(args, tokenizer)
        ...
        for step, sample in enumerate(tqdm(train_dataloader, desc="Train Iteration")):
            ...
            if (step+1) % args.gradient_accumulation_steps == 0:
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    metric = result['macro-f1'] if args.task_name == "FewRel" else result['micro-f1']
                    ...
                    if args.wandb_project:
                        if args.task_name == "TACRED":
                            ...
                            wandb.log({
                                'eval loss': eval_loss,
                                'accuracy': result['accuracy'],
                                'macro-p': result['macro-p'],
                                'macro-r': result['macro-r'],
